...
class GenerateProfileUseCaseTest {
    ...
    @Test
    fun `given profile already exists, when generate profile called, return existing profile`() = runBlocking {
        // given
    ...
    @Test
    fun `given profile does not exist, when generating one, verify correct data generated from mnemonic`() = runBlocking {
        val getMnemonicUseCase = mock<GetMnemonicUseCase> {
            onBlocking { invoke() } doReturn "bright club bacon dinner achieve pull grid save ramp cereal blush woman " +
                    "humble limb repeat video sudden possible story mask neutral prize goose mandate"
        }
        ...
        Assert.assertEquals(profile.factorSources.curve25519OnDeviceStoredMnemonicHierarchicalDeterministicSLIP10FactorSources
            .first().factorSourceID, factorSourceId)

        Assert.assertEquals(profile.perNetwork.first().accounts.first().securityState.unsecuredEntityControl
            .genesisFactorInstance.factorInstanceID, factorInstanceId)
    }

    @Test
    fun `given profile does not exist, when generating one, verify correct data generated from other mnemonic`() = runBlocking {
        val getMnemonicUseCase = mock<GetMnemonicUseCase> {
