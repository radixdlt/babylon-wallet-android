...
internal class PersonaEditViewModelTest : BaseViewModelTest<PersonaEditViewModel>() {
    ...
    fun `delete field updates state`() = runTest {
        ...
        vm.state.whileCollecting {
            advanceUntilIdle()
            vm.onDeleteField(Network.Persona.Field.Kind.Email)
            advanceUntilIdle()
            vm.onSave()
            advanceUntilIdle()
            val persona = slot<Network.Persona>()
    ...
    fun `field value change update state`() = runTest {
        ...
        vm.state.whileCollecting {
            advanceUntilIdle()
            vm.onFieldValueChanged(OnNetwork.Persona.Field.Kind.Email, "jakub@jakub.pl")
            advanceUntilIdle()
            vm.onFieldValueChanged(OnNetwork.Persona.Field.Kind.FirstName, "jakub")
            advanceUntilIdle()
            vm.state.test {
                val item = expectMostRecentItem()
    ...
    fun `field validation works`() = runTest {
        ...
        vm.state.whileCollecting {
            advanceUntilIdle()
            vm.onSelectionChanged(Network.Persona.Field.Kind.LastName, true)
            vm.onSelectionChanged(Network.Persona.Field.Kind.PersonalIdentificationNumber, true)
            vm.onSelectionChanged(Network.Persona.Field.Kind.ZipCode, true)
            vm.onAddFields()
            advanceUntilIdle()
            vm.onDisplayNameChanged("jakub")
            vm.onFieldValueChanged(OnNetwork.Persona.Field.Kind.FirstName, "66666")
            vm.onFieldValueChanged(OnNetwork.Persona.Field.Kind.LastName, "66666")
            vm.onFieldValueChanged(OnNetwork.Persona.Field.Kind.Email, "jakub@jakub.pl")
            ...
            vm.onFieldValueChanged(OnNetwork.Persona.Field.Kind.FirstName, "")
            vm.onFieldValueChanged(OnNetwork.Persona.Field.Kind.LastName, "")
            vm.onFieldValueChanged(OnNetwork.Persona.Field.Kind.Email, "jakubjakub.pl")
