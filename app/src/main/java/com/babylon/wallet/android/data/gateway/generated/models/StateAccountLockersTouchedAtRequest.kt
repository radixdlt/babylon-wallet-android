/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package com.babylon.wallet.android.data.gateway.generated.models

import com.babylon.wallet.android.data.gateway.generated.models.AccountLockerAddress
import com.babylon.wallet.android.data.gateway.generated.models.LedgerStateSelector

import kotlinx.serialization.Serializable
import kotlinx.serialization.SerialName
import kotlinx.serialization.Contextual

/**
 * 
 *
 * @param accountLockers 
 * @param atLedgerState 
 */
@Serializable

data class StateAccountLockersTouchedAtRequest (

    @SerialName(value = "account_lockers")
    val accountLockers: kotlin.collections.List<AccountLockerAddress>,

    @SerialName(value = "at_ledger_state")
    val atLedgerState: LedgerStateSelector? = null

)

