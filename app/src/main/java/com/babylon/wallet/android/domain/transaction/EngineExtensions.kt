...
fun ECKeyPair.toEnginePublicKeyModel(): models.crypto.PublicKey {
    return when (this.publicKey.curveType) {
        EllipticCurveType.Secp256k1 -> {
            models.crypto.PublicKey.EcdsaSecp256k1.fromByteArray(getCompressedPublicKey().removeLeadingZero())
        }
        EllipticCurveType.Ed25519 -> {
            models.crypto.PublicKey.EddsaEd25519.fromByteArray(getCompressedPublicKey().removeLeadingZero())
        }
...
fun PrivateKey.toEngineModel(): models.crypto.PrivateKey {
    return when (this.curveType) {
        EllipticCurveType.Secp256k1 -> {
            models.crypto.PrivateKey.EcdsaSecp256k1.newFromPrivateKeyBytes(this.key.toByteArray())
        }
