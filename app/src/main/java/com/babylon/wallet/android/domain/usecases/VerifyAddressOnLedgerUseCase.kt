...
class VerifyAddressOnLedgerUseCase @Inject constructor(
    ...
    suspend operator fun invoke(address: String): Result<Unit> {
        ...
        if (!account.isLedgerAccount) return Result.failure(Exception("Account $address is not a Ledger backed account"))
        ...
        val ledgerFactorSourceBadge = (factorInstance.badge as? FactorInstance.Badge.VirtualSource.HierarchicalDeterministic)
        ...
        return ledgerMessenger.deriveAndDisplayAddressRequest(
            ...
            keyParameters = LedgerInteractionRequest.KeyParameters(
                curve = Curve.from(ledgerFactorSourceBadge.publicKey.curve),
                derivationPath = ledgerFactorSourceBadge.derivationPath.path
            ),
            ledgerDevice = LedgerInteractionRequest.LedgerDevice.from(ledgerFactorSource)
        ).fold(
            onSuccess = { response ->
                if (response.derivedAddress.derivedKey.publicKeyHex != ledgerFactorSourceBadge.publicKey.compressedData) {
